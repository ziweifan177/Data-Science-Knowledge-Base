...
try:
    ...
    char_len=100
    if opts.char_len is not None:
        try:
            char_len=int(opts.char_len)
        except:
            sys.exit(5)
            
    # parser.add_option('-z', '--vec_size', help='Vector size for word2vec')
    vec_size=50
    if opts.vec_size is not None:
        try:
            vec_size=int(opts.vec_size)
        except:
            sys.exit(5)
...
#     data_path="/home/ec2-user/Prabhu/word2vec/DAT/titanic_train.csv"
    ...
    if word2vec is True:
        ...
        word2vec_df=word2vec_function(samples_df, char_len, vec_size)# samples_df=df
        print(word2vec_df)
